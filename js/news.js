...
class NewsSystem {
    ...
    createNewsContainer() {
        ...
        this.newsContainer.className = 'news-container hidden';
        ...
        this.newsContainer.appendChild(newsHeader);
    ...
    checkForNews() {
        const currentYear = this.game.player.currentYear;
        const currentMonth = this.game.player.currentMonth;
        ...
        for (const newsEvent of NEWS_EVENTS) {
            if (newsEvent.date.year === currentYear && newsEvent.date.month === currentMonth) {
                ...
                if (!this.shownNews.includes(newsId)) {
                    this.showNews(newsEvent);
                    this.shownNews.push(newsId);
                    
                    // Add to game notifications
                    this.game.addNotification(`NEWS: ${newsEvent.title}`);
                    ...
                    // Only show one news event per month
                    break;
                }
            }
        }
    ...
    showNews(newsEvent) {
        ...
        newsContent.innerHTML = `
            ...
            <p>${newsEvent.content}</p>
            <p class="news-date">${GAME_DATA.config.monthNames[newsEvent.date.month]} ${newsEvent.date.year}</p>
        `;
        ...
        if (this.game.state === GameState.RUNNING || this.game.state === GameState.FAST) {
    ...
    hideNews() {
        ...
        if (this.previousState) {
            this.game.setGameState(this.previousState);
